...
@app.route('/')
def home():
    return render_template('index.html',value="No Audio")
...
def results():
    ...
    if request.method == 'POST':
        try:
          ...
          return render_template('index.html', value="No Audio",f="")
...
#    y_pred=model.predict(np.array([x_test]))
    #os.remove(wav_file_pre)
    return render_template('index.html', value=observed_emotions[pred[0]],f=filename)#y_pred[0])
    print(y_pred)
